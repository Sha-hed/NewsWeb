...
const NewsCard = ({news}) => {
    ...
    return (
        <div>
            ...
            <h3 className="text-bold text-red-400 mt-10"> Title : {title}</h3>
            <p className="text-pink-600 text-lg font-bold">{details.length}</p>
            {
                details.length > 200 ? 
                <p>{details.slice(0,200)} <Link  className="font-bold text-green-600" to={`/anews/${_id}`}>Read more</Link></p>:
                <p>{details}</p>
            }
